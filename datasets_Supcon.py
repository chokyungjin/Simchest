...
class ClassPairDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.fov is not None:
            ...
            supcon_base_img = np.array(Image.open(self.samples['imgs'][idx][0]))
            if self.margin is not None:
            ...
            supcon_pair_img = np.array(Image.open(self.samples['imgs'][idx][1]))
        else:
        ...
        supcon_base_img = self.transform(image=supcon_base_img)['image']
        supcon_base_img = self._catch_exception(supcon_base_img)
        supcon_pair_img = self.transform(image=supcon_pair_img)['image']
        supcon_pair_img = self._catch_exception(supcon_pair_img)
